    ...
    return Controller.extend('sap.ui.demo.chartdemo.controller.Detail', {
      ...
      switchColorByColorIndex: function (colorIndex) {
        if (colorIndex === 'Default_Color') {
          this.oVizFrame.setVizProperties({
            plotArea: {
              colorPalette:
                DefaultPropertiesHelper._general.plotArea.colorPalette
            }
          })
        }
        if (colorIndex === 'Semantic_Color') {
          this.oVizFrame.setVizProperties({
            plotArea: {
              colorPalette: ['#6A5ACD', '#483D8B', '#8470FF', '#848f94']
            }
          })
        }
      },
      ...
      getFeedItem: function (measureIndex) {
        if (!this.isExist(measureIndex)) {
          measureIndex = ['Lucro', 'Custo', 'Receita']
        }
      ...
      onInit: function () {
        ...
        this.chartPopover.setActionItems()
        this.chartPopover.connect(this.oVizFrame.getVizUid())
        this.oVizFrame.attachEventOnce(
          'renderComplete',
          function () {
            this.colorPalette =
              this.oVizFrame.getVizProperties().plotArea.colorPalette
          }.bind(this)
        )
      },
      attachContentChange: function (evt) {
        ...
        MessageToast.show(
          'O evento Alteração de conteúdo foi disparado. O item selecionado foi alterado para:' +
            itemId
        )
      }
    })
